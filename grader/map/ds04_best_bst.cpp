...
class map_bst {
 ...
 public:
  ...
  map_bst<KeyT, MappedT, CompareT>& operator=(
      map_bst<KeyT, MappedT, CompareT> other) {
...
void gen_best_bst(int l, int r, CP::map_bst<int, int>& bst) {
  if (r < l) return;
  int m = (l + r) >> 1;
  bst[m] = 1;
  gen_best_bst(l, m - 1, bst);
  gen_best_bst(m + 1, r, bst);
}
