...
void vector_swap(vector<int> &v1, vector<int> &v2, int start1, int end1,
  ...
  for (int i = start1; i < end1; i++) tmp1.push_back(v1[i]);
  for (int i = start2; i < end2; i++) tmp2.push_back(v2[i]);
  v1.erase(v1.begin() + start1, v1.begin() + end1);
  v2.erase(v2.begin() + start2, v2.begin() + end2);
  v1.insert(v1.begin() + start1, tmp2.begin(), tmp2.end());
  v2.insert(v2.begin() + start2, tmp1.begin(), tmp1.end());
}
int main() {
  // read input
  int n, c;
